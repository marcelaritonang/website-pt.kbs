      ...
      <div className="container mx-auto px-4">
        <div className="max-w-3xl mx-auto mt-12">
          ...
          >
            ...
            />
            <div className="ml-4">
              <h3 className={`text-lg font-semibold ${isDark ? 'text-white' : 'text-gray-900'}`}>{preConstructionArticle.author}</h3>
              <p className={`text-sm ${isDark ? 'text-gray-400' : 'text-gray-600'}`}>{preConstructionArticle.authorRole}</p>
            </div>
          </motion.div>
          ...
          >
            <div className="flex flex-wrap gap-2">
              ...
              {preConstructionArticle.tags.map((tag, index) => (
                <span 
                  key={index}
                  className={`text-sm ${isDark ? 'bg-gray-700 text-gray-300' : 'bg-gray-100 text-gray-700'} px-3 py-1 rounded-full`}
                >
                  {tag}
                </span>
              ))}
